...
(setq custom-file (cat-user-file "custom.el"))
(let ((cat-custom-source (cat-config-file "custom.el")))
  (when (file-exists-p cat-custom-source)
    (load cat-custom-source nil 'nomessage)))
